...
public class ListExample {
	...
	public static void main(String[] args) {
		...
		// ArrayList와 LinkedList의 사용양식은 전부 List를 따른다.
		// 따라서 교환이 가능하다. 인터페이스가 같은 구현체들은 사용법이 같다.
		// 제네릭 : 타입 안정화를 위해 객체 내부에 저장되는 타입을 일치시키는 방법
		// String만 저장할 수 있는 컬렉션
		List<String> list = new ArrayList<>(); // 1.8부터 생성자에 제네릭 표기 안해도됨
		List<String> list2 = new LinkedList<>(); // 1.8부터 생성자에 제네릭 표기 안해도됨
		
		String str1 = "Java";
		String str2 = "SpringBoot";
		System.out.println(list); 
		// 컬렉션 특성상 toString()이 이미 재정의되어 주소가 안나오고 내부요소가 나옴
		...
		int size = list.size();
		System.out.println("list1에 저장된 총 객체의 수 : " + size);
		...
		// list 내부에 저장된 객체 인덱스 번호 확인하기
		int idx = list.indexOf("Networ"); // 없는 자료 조회시, -1
		System.out.println(idx);
		
		// list에서 객체 데이터 삭제 remove(index), remove(객체)
		list.remove(5); // 5번 문자열 Database 삭제
		System.out.println(list);
		list.remove(str1); // list에서 str1(Java) 삭제
		System.out.println(list);
		...
		System.out.println("=================");
		// 향상된 for문도 사용가능(List<String>으로 선언했으므로, String으로 받기)
		for(String str : list) {
