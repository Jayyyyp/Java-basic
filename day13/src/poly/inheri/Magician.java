...
public class Magician {
	...
	public void castFireball(Monster monster){
	    ...
	    // 공격력이 2배인 파이어볼 스킬
	    int fireballDamage = this.atk * 2;
		// 죽은 몬스터가 다음 공격에 죽으면 플레이어에게 겸치
	 	if(monster.isInactiveAfterNextAttacked(fireballDamage)) {
	 		callUserAttackMonsterActive(monster);
	 			}
	 			// 몬스터가 다음 공격에 죽지않으면 반격하여 체력 차감
	 	else {
	 			monster.setAfterAttackedHp(fireballDamage);
